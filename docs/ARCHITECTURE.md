...
Developer setup, URL/search contract, regen pitfalls, and a troubleshooting table live in **[docs/DEVELOP.md](DEVELOP.md)**.

---

## Regenerating `commands.json`

`parse.py` **overwrites** `commands.json` from `scripts/sources/` (gitignored) plus the checked-in `scripts/*.json` intermediates. It does **not** keep later DCN / modern-ops / thin-vendor rows unless those passes are re-run afterwards. Day-to-day UI work should leave the committed corpus alone — see [DEVELOP.md](DEVELOP.md).
